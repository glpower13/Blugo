...
**Nachtrag 2026-07-23 (abends) — Fehler des Cloud-Adapters sichtbar machen:** Der direkte Browser-Aufruf (`fetch` an `api.anthropic.com`, Header `anthropic-dangerous-direct-browser-access: true`) verschluckte bei einem HTTP-Fehler die **echte** Anbieter-Meldung — ein `404` (Modell für diesen Schlüssel nicht freigeschaltet) erschien nur als „HTTP 404". Ergebnis: „die Cloud-KI geht nicht", ohne Grund. Entscheidung: die drei Adapter (Decoder/Explainer/Generator) teilen sich **einen** Aufruf-Helfer (`callMessages`, `adapters/anthropic.ts`), der den Body `{error:{message}}` liest und **durchreicht**; `400`/`404` bekommen einen handlungsleitenden Hinweis („anderes Modell wählen"), Netzwerk/CORS eine konkrete Meldung (Ad-Blocker/Firmennetz). Keine Sampling-Parameter (Opus 4.8/Sonnet 5 lehnen sie ab). So bleibt der BYOK-Pfad **diagnostizierbar**, statt still zu scheitern.
