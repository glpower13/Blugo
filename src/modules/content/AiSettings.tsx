    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-2xl bg-surface p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-base font-semibold text-slate-100">KI-Einstellungen</h2>
          <button onClick={close} className="text-slate-400" aria-label="Schließen">
            ✕
          </button>
        </div>
        ...
        {isCloud && (
          <div className="mt-4 flex flex-col gap-3">
            <label className="flex flex-col gap-1">
              <span className="text-sm text-slate-200">Dein Claude-Zugangs-Schlüssel</span>
              <input
                type="password"
                ...
                autoComplete="off"
                spellCheck={false}
                className="rounded-lg border border-slate-600 bg-base px-3 py-2 text-slate-100"
              />
            ...
            <label className="flex flex-col gap-1">
              <span className="text-sm text-slate-200">Modell (Qualität ↔ Kosten)</span>
              <select
            ...
            {test.state === 'ok' && (
              <p className="text-xs text-emerald-400">✓ Klappt: {test.msg}</p>
            )}
            {test.state === 'error' && <p className="text-xs text-rose-300">✕ {test.msg}</p>}
        ...
        <div className="mt-5 flex gap-2">
          <button
            onClick={save}
            className="flex-1 rounded-lg bg-brand px-4 py-2 font-medium text-white"
          >
            Speichern
          </button>
          <button
  ...
  return (
    <button
      ...
      className={`rounded-xl border p-3 text-left ${
        active ? 'border-brand bg-brand/10' : 'border-slate-600'
      }`}
    >
